...
USE_FP16 = True
USE_CUDAGRAPH = False
controlnet_engine_file_path = "./engines/controlnet.plan"
controlunet_engine_file_path = "./engines/controlunet.plan"
clip_engine_file_path = "./engines/clip.plan"
vae_engine_file_path = "./engines/vae_encoder.plan"
...
def trt_setup():
    ...
    clip_engine = read_engine(clip_engine_file_path)
    clip_context, clip_stream = set_clip_context(clip_engine)
    vae_engine = read_engine(vae_engine_file_path)
    vae_context, vae_stream = set_vae_context(vae_engine)
...
def set_clip_context(clip_engine):
        engine = clip_engine
        ...
        context.set_input_shape(lTensorName[0], [1, 77])
        # for i in range(nIO):
        #     print("[%2d]%s->" % (i, "Input " if i < nInput else "Output"), engine.get_tensor_dtype(lTensorName[i]), engine.get_tensor_shape(lTensorName[i]), context.get_tensor_shape(lTensorName[i]), lTensorName[i])
        return context, stream

clip_temp = 0
def trt_clip_run(tokens):
        global clip_engine
        global clip_context
        global clip_stream
        global clip_graphExe
        global clip_temp
        ...
        bufferH = []
        bufferH.append(np.ascontiguousarray(tokens.cpu().numpy()))
        ...
        if False:
            ...
            if CAPTURE:        
                ...
                clip_graphExe = graphExe
                clip_temp = 1
                return torch.tensor(bufferH[-1]).cuda()
            else:
                cudart.cudaGraphLaunch(clip_graphExe, stream)
...
def trt_vae_run(z):
        ...
        bufferH = []
        bufferH.append(np.ascontiguousarray(z.cpu().numpy()))
